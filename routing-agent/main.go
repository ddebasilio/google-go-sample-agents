...
func main() {
	...
	wandSpecialist, err := llmagent.New(llmagent.Config{
		Name:        "WandSpecialist",
		Model:       model,
		Description: "Answers questions about wand products: woods, cores, lengths, what to buy, how wands differ.",
...
say so and suggest they speak to a magical technician instead.`,
	...
	magicalTechnician, err := llmagent.New(llmagent.Config{
		Name:        "MagicalTechnician",
		Model:       model,
		Description: "Diagnoses and repairs malfunctioning wands: wrong spells firing, damage, contamination, unresponsive cores.",
...
either a transfer call (A/B) or a short text reply (C).`,
		SubAgents: []agent.Agent{wandSpecialist, magicalTechnician},
	})
	if err != nil {
		panic(err)
	}
